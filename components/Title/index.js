...
const ParallaxLetter = ({ letter }) => {
    const yOff = 15 + getRandomInt(-100, 50);
    ...
    return (
        <div className='relative flex-auto'>
            {letter.forms.map((X, i) => (
                <Parallax
                    key={i}
                    y={[-yOff * (i + 1) + 'px', yOff * 0.25 * (i + 1) + 'px']}
                    children={(
                        <div className='absolute transform -translate-y-1/2 xl:p-8 md:p-4 p-1'>
                            {X}
                        </div>
                    )}
                />
...
export default () => (
    <div id='title' className='relative'>
        <ParallaxBanner
            ...
            children={(
                <div className='z-10 block h-full'>
                    <ParallaxWord />
                    <div className='pb-10 flex flex-col h-full justify-end items-center'>
                        <Jump>
                            <a href='https://github.com/Monolith-Non-Corp' className='hover:opacity-80'>
